...
def do(prefix, outdir, version, description):
  ...
  dir = outdir
  patchinfo_json = outdir+'/{0}_info.json'.format(prefix)
  patchinfo_json_lock = outdir+'/{0}_info.json.lock'.format(prefix)
  ...
  for i in range(len(revisionFilesetList)):
    zipname = dir + '/{0}_{1}.zip'.format(prefix, str(i))
  ...
  for revision in revisions:
    ...
    # patches[revision] = {'idx':idx, 'hash': revisionFilesetHashList[idx], 'removes':list(getDiffDeletedFileSetToHead(revision)), 'size': revisionFilesetSizeList[idx], 'path': headRevision +'/{0}_{1}.zip'.format(prefix, str(idx))}
    patches[revision] = {'idx':idx, 'hash': revisionFilesetHashList[idx], 'removes':list(getDiffDeletedFileSetToHead(revision)), 'size': revisionFilesetSizeList[idx], 'path': '{0}_{1}.zip'.format(prefix, str(idx))}
...
if __name__ == '__main__':
  ...
  if len(sys.argv) < 4:
    print("patch_gen.py [src] [patch] [prefix]")
    sys.exit(0)
  #do('./patch')
  ...
  os.chdir(dir)
  with lock:
    do(prefix, outdir, version, description)
  os.chdir(dir)
